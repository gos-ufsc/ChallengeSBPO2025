...
#example = "datasets/b/instance_0014.txt"
        ...
        for i in range(n_pedidos):
            if pedido_X[i].x > 0:
                ...
                if i not in pedidos_fora:
                    ...
                    pedido_X[i].LB = 0
                    pedido_X[i].UB = 0
        conjuntos_match.append((p_array, c_array, pontuacao, itens_casal))
        if valid:
...
print()
print('########## Inicialização da parte 3 ###########')
model2 = gp.Model()
model2.setParam('OutputFlag', 0)  # Desativa os prints 

#vars

casal = model2.addVars(len(conjuntos_match), vtype=GRB.BINARY, name="casal")
#casal_valido = model2.addVars(len(casais_validos), vtype=GRB.BINARY, name="casal_valido")
#quase_casal = model2.addVars(len(casais_invalidos), vtype=GRB.BINARY, name="quase_casal")

model2.setObjective(gp.quicksum(conjuntos_match[i][2] * casal[i] for i in range(len(conjuntos_match))), GRB.MAXIMIZE)

#constr

model2.addConstr(gp.quicksum(casal[i]*conjuntos_match[i][2] for i in range(len(conjuntos_match))) <= UB)
model2.addConstr(gp.quicksum(casal[i]*conjuntos_match[i][2] for i in range(len(conjuntos_match))) >= LB)

model2.optimize()

if model2.status == GRB.OPTIMAL:
    contar = 0
    for i in range(len(conjuntos_match)):
        if casal[i].x > 0:
            contar += len(conjuntos_match[i][1])
    print()
...
model3 = gp.Model()
model3.setParam('OutputFlag', 0)  # Desativa os prints 

corredores_selecionados = set()
itens_solucao_anterior = [0]*n_itens

for i in range(len(conjuntos_match)):
    if casal[i].x > 0:
        for iter_itens in range(n_itens):
            itens_solucao_anterior[iter_itens] += conjuntos_match[i][3][iter_itens]
        for i in conjuntos_match[i][1]:
            corredores_selecionados.add(i)
negativos = 0
for i in range(n_itens):
    if itens_solucao_anterior[i] < 0:
        negativos += 1
print(f'negativos: {negativos}')

var_corredor = model3.addVars(n_corredores, vtype=GRB.BINARY, name="corredor_Y")
# fixando os corredores já selecionados
for i in range(n_corredores):
    if i in corredores_selecionados:
        var_corredor[i].LB = 1
        var_corredor[i].UB = 1

model3.setObjective(gp.quicksum(var_corredor[i] for i in range(n_corredores)), GRB.MINIMIZE)
