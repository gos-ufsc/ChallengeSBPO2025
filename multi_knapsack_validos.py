...
model2.setObjective(gp.quicksum(casais_validos[i][2] * casal[i] for i in range(len(casais_validos))), GRB.MAXIMIZE)

#constr

model2.addConstr(gp.quicksum(casal[i]*casais_validos[i][2] for i in range(len(casais_validos))) <= UB)
 
# Removi o lower para ter uma solução de cara
#model2.addConstr(gp.quicksum(casal[i]*casais_validos[i][2] for i in range(len(casais_validos)))) >= LB 
